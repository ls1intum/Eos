...
public class UnwantedNode {
    ...
    public static List<UnwantedNode> getUnwantedNodesInJavaFile(JavaFile javaFile, Map<String, Class<? extends Node>> nodesDefinedAsUnwanted) {
        return nodesDefinedAsUnwanted.keySet().stream().map(unwantedNodeName -> new UnwantedNode(javaFile, unwantedNodeName, nodesDefinedAsUnwanted.get(unwantedNodeName)))
                .filter(unwantedNode -> !unwantedNode.getUnwantedNodePositions().isEmpty()).sorted(Comparator.comparing(uwn -> uwn.getUnwantedNodePositions().get(0)))
    ...
    public static String getFormattedUnwantedNodeString(UnwantedNode unwantedNode) {
        return unwantedNode.getUnwantedNodePositions().stream().map(String::valueOf).map(UnwantedNode::getFormattedPositionString).collect(Collectors.joining(
                System.lineSeparator(), localized("ast.method.get_formatted_unwanted_node_string_prefix", unwantedNode.getUnwantedNodeName()) + System.lineSeparator(), ""));
    }
    ...
    public static String getFormattedFileString(Path filePath, Map<Path, List<UnwantedNode>> unwantedNodes) {
        return unwantedNodes.get(filePath).stream().map(UnwantedNode::getFormattedUnwantedNodeString)
                .collect(Collectors.joining(System.lineSeparator(), localized("ast.method.get_formatted_file_string_prefix", filePath) + System.lineSeparator(), ""));
    }
    ...
    public static Optional<String> getMessageForUnwantedNodesForFileAt(Path pathOfJavaFile, Map<String, Class<? extends Node>> nodeNameUnwantedNodeMap) {
        ...
        return Optional.of(unwantedNodes.keySet().stream().map(filePath -> getFormattedFileString(filePath, unwantedNodes)).reduce(String::concat).orElse(""));
    }
    ...
    public static Optional<String> getMessageForUnwantedNodesForAllFilesBelow(Path pathOfDirectory, Map<String, Class<? extends Node>> nodeNameUnwantedNodeMap) {
        return JavaFile.readFromDirectory(pathOfDirectory).stream().sorted(Comparator.comparing(JavaFile::getJavaFilePath))
                .map(javaFile -> getMessageForUnwantedNodesForFileAt(javaFile.getJavaFilePath(), nodeNameUnwantedNodeMap)).filter(Optional::isPresent).map(Optional::get)
                .map(message -> message + System.lineSeparator()).reduce(String::concat).map(String::trim).map(message -> " " + message);
    }
}
