...
public final class IOExtensionUtils {
    ...
    private static IOManager<?> createIOManagerFor(TestContext testContext) {
        var ioManagerClass = TestContextUtils.findAnnotationIn(testContext, WithIOManager.class).<Class<? extends IOManager<?>>>map(WithIOManager::value)
                .orElse(DEFAULT_IO_MANAGER);
