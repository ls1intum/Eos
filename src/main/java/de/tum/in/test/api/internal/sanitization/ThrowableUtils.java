...
final class ThrowableUtils {
    ...
    static final Set<String> THROWABLE_PROPERTIES = Stream.concat(IGNORE_PROPERTIES.stream(), Stream.of(MESSAGE, STACK_TRACE, CAUSE, SUPPRESSED))
    ...
    static ThrowableCreator getThrowableCreatorFor(Class<? extends Throwable> type, Constructor<? extends Throwable> constructor) {
        ...
        return info -> {
            ...
            var originalPropertyValuesByType = getValuesByPropertyType(propertiesWithMethods.stream().sorted(ORDER_PROPERTIES_BY_RELEVANCE), originalProperties);
    ...
    static <T extends Throwable> Constructor<T> findPreferredConstructor(Class<T> type) {
        ...
        return Stream.of(allConstructors).filter(constructor -> isSatisfiableByProperties(constructor, classPropertyTypeCount)).sorted(getConstructorPreferenceOrder()).findFirst()
                .orElse(allConstructors[0]);
    }

    static boolean isSatisfiableByProperties(Constructor<?> constructor, Map<? extends Class<?>, Long> propertyTypeCount) {
        var parameterTypeCount = Stream.of(constructor.getParameterTypes()).collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
        return parameterTypeCount.entrySet().stream().mapToLong(e -> propertyTypeCount.getOrDefault(e.getKey(), 0L) - e.getValue()).min().orElse(0) >= 0;
    }

    static Comparator<Constructor<?>> getConstructorPreferenceOrder() { // NOSONAR
        return Comparator.<Constructor<?>>comparingInt(Constructor::getParameterCount).thenComparingLong(x -> Stream.of(x.getParameterTypes()).filter(String.class::equals).count())
                .reversed();
    }
    ...
    static Map<String, Object> retrievePropertyValues(Object instance, Set<Entry<String, Method>> properties) {
        return properties.stream().collect(Collectors.groupingBy(Map.Entry::getKey,
                Collectors.mapping(property -> ReflectionSupport.invokeMethod(property.getValue(), instance), Collectors.reducing(null, (a, b) -> a != null ? a : b))));
    }

    static Map<Class<?>, List<Object>> getValuesByPropertyType(Stream<Entry<String, Method>> propertiesWithMethods, Map<String, Object> concretePropertyValues) {
