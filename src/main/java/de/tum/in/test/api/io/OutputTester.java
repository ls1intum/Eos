...
public final class OutputTester implements LineAcceptor {
    ...
    public String getOutputAsString(OutputTestOptions... outputOptions) {
        return processLines(outputOptions).stream().map(Line::text).collect(Collectors.joining(IOTester.LINE_SEPERATOR));
    }
    ...
    private static AssertionFailedError tryCleanUpAssertionFailedError(List<String> lines, AssertionFailedError afe) {
        ...
        if (matcher.find()) {
            ...
            if (startsWithEscape(expectedLine))
                ...
                cleanExpectedLine.delete(0, 2).delete(cleanExpectedLine.length() - 2, cleanExpectedLine.length()).insert(0, " `") //$NON-NLS-1$
                        .insert(0, localized("output_tester.line_matching_regex_mismatch")).append('`');  //$NON-NLS-1$
            else
