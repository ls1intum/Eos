...
public final class ArtemisSecurityManager extends SecurityManager {
    ...
    static {
        ...
        if (Runtime.version().feature() == 17) {
            SecurityConstants.SYSTEM_ERR.format("NOTICE: The warning above is expected and the issue is already known.%n" //$NON-NLS-1$
    ...
    public void checkPermission(Permission perm) {
        ...
        try {
            ...
            if ((configuration == null || configuration.allowLocalPortsAbove().isEmpty()) && (perm instanceof NetPermission || perm instanceof SocketPermission))
            ...
            if ("<<ALL FILES>>".equals(path)) { //$NON-NLS-1$
                ...
                if (commonPath == null) {
                    ...
                    whitelisted = !blacklisted
                            && configuration.whitelistedPaths().orElse(Set.of()).stream().anyMatch(pm -> pm.matchesRecursivelyWithLevel(absolutePath, pathActionLevel));
                }
            }
    ...
    private boolean isPackageAccessForbidden(String pkg) {
        return SecurityConstants.PACKAGE_USE_BLACKLIST.stream().anyMatch(pkg::startsWith) || (isPackageBlacklisted(pkg) && !isPackageWhitelisted(pkg));
    }
    ...
    private List<StackFrame> getNonWhitelistedStackFrames(Predicate<StackFrame> takeFromTopWhileFilter) {
        ...
        if (isCurrentThreadWhitelisted()) {
            result = stackWalker.walk(sfs -> sfs.takeWhile(takeFromTopWhileFilter).filter(this::isStackFrameNotWhitelisted).collect(Collectors.toList()));
        }
        else {
    ...
    private boolean isCallNotWhitelisted(String className, String methodName) {
        ...
        return SecurityConstants.STACK_BLACKLIST.stream().anyMatch(call::startsWith)
                || (SecurityConstants.STACK_WHITELIST.stream().noneMatch(call::startsWith) && (configuration == null
                        || (!configuration.whitelistedClassNames().contains(className) && configuration.trustedPackages().stream().noneMatch(pm -> pm.matches(className)))));
    }
    ...
    private boolean isLocalPortUsageAllowed(int port) {
        ...
        return configuration.allowedLocalPorts().contains(port)
                || (configuration.allowLocalPortsAbove().orElse(MAX_PORT) < port && !configuration.excludedLocalPorts().contains(port));
    }
    ...
    private void checkCommonThreadPool() {
        ...
        var threads = Stream.of(allThreads).filter(Objects::nonNull).filter(t -> t.getName().contains(COMMON_POOL_THREAD_NAME)).toArray(Thread[]::new);
    ...
    public static synchronized void uninstall(String accessToken) {
        ...
        if (activeThreads.length > 0)
            throw new IllegalStateException(localized("security.error_threads_still_active", Arrays.toString(activeThreads))); //$NON-NLS-1$
    }
