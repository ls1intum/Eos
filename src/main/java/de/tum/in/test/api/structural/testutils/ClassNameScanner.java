    ...
    private ScanResult computeScanResult() {
        ...
        for (var observedClass : observedClasses.entrySet()) {
            ...
            if (foundObservedClassName.equalsIgnoreCase(expectedClassName))
                return createScanResult(foundObservedClassName, foundObservedPackageNames, classPresentMultiple, classCorrectlyPlaced, WRONG_CASE_MULTIPLE,
                        WRONG_CASE_CORRECT_PLACE, WRONG_CASE_MISPLACED);
            /*
            ...
            if (isMisspelledWithHighProbability(expectedClassName, foundObservedClassName))
                return createScanResult(foundObservedClassName, foundObservedPackageNames, classPresentMultiple, classCorrectlyPlaced, TYPOS_MULTIPLE, TYPOS_CORRECT_PLACE,
                        TYPOS_MISPLACED);
        }
        return createScanResult(ScanResultType.NOTFOUND, expectedClassName, null);
    }

    private ScanResult createScanResult(String foundObservedClassName, String foundObservedPackageName, boolean classPresentMultiple, boolean classCorrectlyPlaced,
            ScanResultType multipleTimes, ScanResultType correctPlace, ScanResultType misplaced) {
